...
// Handle the project data import safely
let projectIcons: string[] = []
try {
  // Use dynamic import for JSON files
  const projectData = await import('./src/content/projects/data.json')
  projectIcons = projectData.default?.map((item: any) => item.icon) || []
} catch (error) {
  console.warn('Projects data file not found, using empty array for icons')
  projectIcons = []
}
